...
srun torchrun \
  ...
  train_tryongan.py --dataset triplet --triplet_train_data_path ${TRIPLET_TRAIN_DIR} --triplet_test_data_path ${TRIPLET_TEST_DIR} --curriculum none --max_steps 30000 --batch_size 16 --num_workers 16 --gender all --save_interval 1000 --image_log_interval 1000 --skip_eval --no_resume --run_name Stable_diffusion_train_triplet_only_tryongan
