...
srun torchrun \
  ...
  train_tryongan.py --curriculum none --max_steps 30000 --curvton_data_path ${DATA_DIR} --curvton_test_data_path ${TEST_DIR} --triplet_test_data_path ${TRIPLET_DIR} --batch_size 16 --num_workers 16 --gender all --save_interval 1000 --image_log_interval 500 --skip_eval --no_resume --run_name Stable_diffusion_train_no_curriculum_tryongan
