...
srun torchrun \
  ...
  train_DiT.py --curriculum soft --stage_steps 4000 --max_steps 48000 --curvton_data_path ${DATA_DIR} --curvton_test_data_path ${TEST_DIR} --triplet_test_data_path ${TRIPLET_DIR} --batch_size 16 --num_workers 16 --gender all --save_interval 1000 --image_log_interval 500 --skip_eval --no_resume --run_name Stable_diffusion_train_hunyuan
