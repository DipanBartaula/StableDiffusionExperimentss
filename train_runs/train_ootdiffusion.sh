...
# Usage:
#   sbatch train_ootdiffusion.sh --run_nodes 1
#   sbatch train_ootdiffusion.sh --run_nodes 2
...
srun --nodes=${RUN_NNODES} --ntasks=${RUN_NNODES} --ntasks-per-node=1 bash -c '
  torchrun \
    ...
    cross-architecture/OOTDiffusion/train_ootdiffusion_local.py --curvton_data_path '"${DATA_DIR}"' --batch_size 4 --image_size 0 --num_workers 16 --max_steps 28000 --wandb_project Stable_diffusion --save_interval 1000 --image_log_interval 500 --output_dir '"${OUT_DIR}"' --no_resume --run_name Stable_diffusion_train_ootdiffusion
'
