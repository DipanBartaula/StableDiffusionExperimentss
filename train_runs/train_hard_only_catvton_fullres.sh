...
srun torchrun --nnodes=2 --nproc_per_node=4 --node_rank="${SLURM_NODEID}" --master_addr="${MASTER_ADDR}" --master_port=$MASTER_PORT \
  train.py --dataset curvton --difficulty hard --max_steps 28000 --curvton_data_path ${DATA_DIR} --batch_size 4 --num_workers 16 --gender all --image_size 0 --save_interval 1000 --image_log_interval 500 --use_dream --dream_lambda 10.0 --skip_eval --no_resume --run_name Stable_diffusion_train_hard_only_catvton_fullres
