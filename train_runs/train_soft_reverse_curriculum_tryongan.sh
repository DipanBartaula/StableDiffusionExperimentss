#!/bin/bash
#SBATCH --job-name=softrev_curric_gan
#SBATCH --nodes=1
...
srun torchrun \
  ...
  train_tryongan.py --curriculum soft_reverse --stage_steps 10000 --max_steps 30000 --phase2_data_path ${PHASE2_DIR} --phase2_start_step 28801 --curvton_data_path ${DATA_DIR} --curvton_test_data_path ${TEST_DIR} --triplet_test_data_path ${TRIPLET_DIR} --batch_size 16 --num_workers 16 --gender all --save_interval 1000 --image_log_interval 1000 --skip_eval --no_resume --run_name Stable_diffusion_train_soft_reverse_curriculum_tryongan
