#!/bin/bash
#SBATCH --job-name=none_cat_14400_fullres
#SBATCH --nodes=2
...
srun --nodes=${RUN_NNODES} --ntasks=${RUN_NNODES} --ntasks-per-node=1 bash -c '
  torchrun \
    ...
    train.py --dataset curvton --curriculum none --stage_steps 9600 --max_steps 14400 --curvton_data_path '"${DATA_DIR}"' --batch_size 4 --num_workers 16 --gender all --image_size 0 --save_interval 1000 --image_log_interval 500 --skip_eval --run_name Stable_diffusion_train_no_curriculum_catvton_14400steps_fullres
'
