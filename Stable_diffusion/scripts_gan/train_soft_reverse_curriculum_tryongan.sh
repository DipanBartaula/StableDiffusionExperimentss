...
srun torchrun \
  ...
  train_tryongan.py --curriculum soft_reverse --stage_steps 10000 --max_steps 34000 --phase2_data_path ${PHASE2_DIR} --phase2_start_step 28801 --curvton_data_path ${DATA_DIR} --curvton_test_data_path ${TEST_DIR} --triplet_test_data_path ${TRIPLET_DIR} --batch_size 16 --num_workers 8 --gender all --save_interval 1000 --image_log_interval 250 --skip_eval --run_name train_soft_reverse_curriculum_tryongan
